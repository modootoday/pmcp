...
it("reads the installed version instead of the requested range", () => {
  ...
  json(join(path, "node_modules/example/package.json"), { name: "example", version: "2.3.4" });
  expect(readInstalledDependencies(path)).toEqual({
    dependencies: [{ name: "example", requestedAs: "example", version: "2.3.4" }], issues: [],
  });
});
...
it("resolves hoisted manifests without importing package code or exports", () => {
  ...
  json(join(path, "node_modules/@example/ui/package.json"), {
    name: "@example/ui", version: "1.2.0", exports: { ".": "./does-not-exist.js" },
  });
  expect(readInstalledDependencies(project).dependencies[0]?.version).toBe("1.2.0");
});

it("reports missing packages rather than using the declared version", () => {
  const path = root();
  json(join(path, "package.json"), { optionalDependencies: { "pmcp-fixture-missing": "1.0.0" } });
  expect(readInstalledDependencies(path)).toEqual({ dependencies: [], issues: [
    { name: "pmcp-fixture-missing", reason: "not_installed" },
  ] });
});
...
it("does not bypass a corrupt local manifest for a hoisted copy", () => {
  ...
  json(join(path, "node_modules/example/package.json"), { name: "example", version: "1.0.0" });
  json(join(project, "node_modules/example/package.json"), {});
  expect(readInstalledDependencies(project)).toEqual({ dependencies: [], issues: [
    { name: "example", reason: "invalid_manifest" },
  ] });
});
...
it("preserves aliases and deduplicates declarations across fields", () => {
  ...
  json(join(path, "package.json"), {
    dependencies: { alias: "npm:example@1" }, devDependencies: { alias: "npm:example@1" },
  });
  json(join(path, "node_modules/alias/package.json"), { name: "example", version: "1.0.0" });
...
it("rejects path-shaped names before resolving any dependency", () => {
  ...
  expect(() => readInstalledDependencies(path)).toThrow("invalid dependency declaration");
});
