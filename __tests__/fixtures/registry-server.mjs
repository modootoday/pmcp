...
const server = createServer(tls, (request, response) => {
  if (process.env.LOG) appendFileSync(process.env.LOG, `${JSON.stringify({ path: request.url, authorization: request.headers.authorization?.replace(/pmcp_[A-Za-z0-9_-]+/g, "[redacted]") ?? null })}\n`);
  ...
  if (!accepted.has(request.headers.authorization ?? "")) {
    response.writeHead(401, {
      ...
      "www-authenticate": 'Basic realm="pmcp npm registry", Bearer realm="pmcp npm registry"',
    });
  ...
  if (path === "/@pmcp/example" || path === "/@pmcp/example/1.0.0") {
    const version = {
      name: "@pmcp/example", version: "1.0.0",
    ...
    response.end(JSON.stringify(path.endsWith("/1.0.0") ? version : {
      name: "@pmcp/example", "dist-tags": { latest: "1.0.0" }, versions: { "1.0.0": version },
    }));
    return;
  }
