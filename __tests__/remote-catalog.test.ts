...
import { catalogEndpoint, fetchCatalog, MAX_CATALOG_BYTES, parseCatalog } from "../src/remote/catalog.js";

const response = () => ({
  schemaVersion: 1, requestId: "test", catalog: {
    revision: "r1", publishedAt: "2026-09-06T00:00:00.000Z", entries: [{
      productId: "example", title: "Example", summary: "Use example correctly.",
      delivery: { packageName: "@pmcp/example", version: "1.0.0", integrity: `sha512-${"A".repeat(86)}==` },
      skillRevision: 1, targets: [{ packageName: "example", range: "^2.0.0", verifiedVersions: ["2.1.0"] }],
...
it("accepts the public wire contract and strips unknown internal fields", () => {
  expect(JSON.stringify(parseCatalog(response()))).not.toContain("must-not-be-returned");
...
it("never puts credentials or local dependency data in a public index request", async () => {
  ...
  expect((await fetchCatalog("https://api.example.com", fake)).revision).toBe("r1");
...
it("counts actual streamed bytes even without a content-length header", async () => {
  ...
  const fake = (async () => new Response(new ReadableStream({
    start(controller) { controller.enqueue(new Uint8Array(MAX_CATALOG_BYTES + 1)); },
    cancel() { cancelled = true; },
  }), { headers: { "content-type": "application/json" } })) as typeof fetch;
  await expect(fetchCatalog("https://api.example.com", fake)).rejects.toThrow("exceeds");
  expect(cancelled).toBe(true);
});

it("rejects credential-bearing URLs and insecure origins", () => {
  for (const origin of ["http://example.com", "https://user:secret@example.com", "https://example.com/private", "https://example.com/?token=x"]) {
